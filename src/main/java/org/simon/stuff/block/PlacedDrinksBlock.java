...
import org.simon.stuff.StuffTags;
import org.simon.stuff.block.entity.StuffBlockEntities;
import org.simon.stuff.block.entity.SyncedBlockEntity;
...
public class PlacedDrinksBlock extends BlockWithEntity {
    ...
    public ItemStack getPickStack(WorldView world, BlockPos pos, BlockState state) {
        return Stuff.getCrossHairTarget()
                ...
                .flatMap(hitPos -> world.getBlockEntity(pos, StuffBlockEntities.PLACED_DRINK).flatMap(be -> be.getDrink(hitPos)))
                .orElseGet(() -> super.getPickStack(world, pos, state));
    }

    public static boolean canPlace(ItemStack stack) {
        return stack.isIn(StuffTags.Items.PLACEABLE);
    }

    @Override
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        return world.getBlockEntity(pos, StuffBlockEntities.PLACED_DRINK).flatMap(be -> {
    ...
    public void onStacksDropped(BlockState state, ServerWorld world, BlockPos pos, ItemStack stack, boolean dropExperience) {
        world.getBlockEntity(pos, StuffBlockEntities.PLACED_DRINK).ifPresent(be -> {
    ...
    public static ActionResult tryPlace(ItemUsageContext context) {
        ...
        if (state.isOf(StuffBlocks.PLACED_DRINK)) {
        ...
        context.getWorld().setBlockState(blockPos, StuffBlocks.PLACED_DRINK.getDefaultState());
        return context.getWorld().getBlockEntity(blockPos, StuffBlockEntities.PLACED_DRINK).map(be -> {
    ...
    public static class Data extends SyncedBlockEntity {
        ...
        public Data(BlockPos pos, BlockState state) {
            super(StuffBlockEntities.PLACED_DRINK, pos, state);
        }
